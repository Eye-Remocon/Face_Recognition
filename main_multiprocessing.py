...
from multiprocessing import Queue, Process  # 멀티프로세싱 작업
    ...
    while True:
        ...
        if time.time() - start_time >= 1:  # check if 1 sec passed, 1초마다
            ...
            if user_identify.is_registered(face_distances):  # 촬영된 사진에 있는 사람이 등록된 사람인지 확인
                ...
                if q.qsize() == 0:  # 음악이 재생되고 있지 않을 경우
...
if __name__ == '__main__':
    emotion_queue = Queue()  # 인식된 감정을 큐에 저장한다. 
    p1 = Process(target=eye_remocon_service, args=(emotion_queue, ))
    p2 = Process(target=play_music.music_start)
    p1.start()  # eye_remocon_service 메서드 실행
    music_playing = False

    while True:
        if (emotion_queue.empty() is False) and (music_playing is False):  # 감정 큐가 비어있고 음악이 재생되고 있지 않을 경우
            music_playing = True  # 음악이 재생되는 동안 다른 음악이 재생되지 못하도록 설정
            p2 = Process(target=play_music.music_start, args=(emotion_queue.get(), ))
            # 감정 큐에 있는 감정을 dequeue 시키고 감정에 맞는 음악 재생, 이 때 감정 큐는 비어있게 된다.
            p2.start()  # 음악 재생
            p2.join()
            music_playing = False
